...
set RTKCFGFILE  = ${HOME}/cfg/rnx2rtkp.curr.conf 	# it includes all the post-processing elaboration options
set CRDFILE	= ${HOME}/tab/station.pos		# it includes apriori coordinates of different GNSS sites (ROVER inluded)
set CFGFILE 	= ${HOME}/cfg/sites.cfg			# it includes ROVER and MASTER GNSS site names to be used in the elaboration
#********************  TUNING VARIABLES END  ********************
#
# DEFAULT PARAMS
set SITE	= "" # ROVER GNSS SITE NAME (default empty string)
set REFV	= "" # ROVER GNSS SITE NAME (default empty string)
set YEAR  	= `date +%Y`
set DOY   	= `date +%j`
set HOUR  	= `date +%H`
set RATE  	= 30 	#sampling rate
set FORCE 	= "N" 	# do not overwrite existing results
